...
def adjustbinstr(binstr, end1, end2):
	lst = list(binstr);
	lstsize = len(lst);
...
def puf_kernel(matkernel, puf, ratio):
...
def puf_conv1_kernel(kernel, puf, ratio): #conv1kernel: 11 * 11
	pesize = 3;
	kernelsize = 11;
	arraysize = 3;
	...
	for i in range (0, arraysize):
		for j in range (0, arraysize):
			...
			kernel_out_array[3 * i : 3 * i + 3, 3 * j : 3 * j + 3] = puf_kernel(matkernel, puf, ratio);
	...
	kernel_out1 = puf_kernel(out1.reshape((3, 3)), puf, ratio)
	kernel_out2 = puf_kernel(out2.reshape((3, 3)), puf, ratio)
	kernel_out3 = puf_kernel(out3.reshape((3, 3)), puf, ratio)
	kernel_out4 = puf_kernel(out4.reshape((3, 3)), puf, ratio)
...
def puf_layer(w, puf, ratio, layer):
	w_shape = np.shape(w);
	...
	if(layer == "conv1"):
		for i in range (0, w_shape[0]):
			for j in range (0, w_shape[1]):
				...
				cur_kernel = w[i][j];
				w_puf[i][j] = puf_conv1_kernel(cur_kernel, puf[puf_row][puf_col], ratio);
		...
		for i in range (0, w_shape[0]):
			for j in range (0, w_shape[1]):
				...
				cur_kernel = w[i][j];
				w_puf[i][j] = puf_kernel(cur_kernel, puf[puf_row][puf_col], ratio);
...
def puf_alexnet(netinpath, netoutpath, ratio):
	net = caffe.Net(caffe_root + 'models/bvlc_alexnet/train_val.prototxt',
	               caffe_root + netinpath,    
	              caffe.TEST);

	pufs_conv1 = np.load("/home/gql/Desktop/debug/reliabilityadjust/pufs_conv1.npy");
	pufs_conv2 = np.load("/home/gql/Desktop/debug/reliabilityadjust/pufs_conv2.npy");
	pufs_conv3 = np.load("/home/gql/Desktop/debug/reliabilityadjust/pufs_conv3.npy");
	pufs_conv4 = np.load("/home/gql/Desktop/debug/reliabilityadjust/pufs_conv4.npy");
	pufs_conv5 = np.load("/home/gql/Desktop/debug/reliabilityadjust/pufs_conv5.npy");
	...
	pw_conv1 = puf_layer(w_conv1, pufs_conv1, ratio, "conv1");
	print "end1"
	pw_conv2 = puf_layer(w_conv2, pufs_conv2, ratio, "conv2");
	print "end2"
	pw_conv3 = puf_layer(w_conv3, pufs_conv3, ratio, "conv3");
	print "end3"
	pw_conv4 = puf_layer(w_conv4, pufs_conv4, ratio, "conv4");
	print "end4"
	pw_conv5 = puf_layer(w_conv5, pufs_conv5, ratio, "conv5");
	print"end5"
...
if __name__=="__main__":
	netinpath = 'models/bvlc_alexnet/pufmodel/e5/bvlc_alexnet_baseline.caffemodel'
	netadjpath =  'models/bvlc_alexnet/pufmodel/e5/bvlc_alexnet_adj.caffemodel'
	netpufpath = 'models/bvlc_alexnet/pufmodel/e5/bvlc_alexnet_puf.caffemodel'
	netoripath = 'models/bvlc_alexnet/pufmodel/e5/bvlc_alexnet_ori.caffemodel'
	...
	pw_conv1 = adjust_layer(w_conv1, pufs_conv1, "conv1");
	pw_conv2 = adjust_layer(w_conv2, pufs_conv2, "conv2");
	pw_conv3 = adjust_layer(w_conv3, pufs_conv3, "conv3");
	pw_conv4 = adjust_layer(w_conv4, pufs_conv4, "conv4");
	pw_conv5 = adjust_layer(w_conv5, pufs_conv5, "conv5");
	...
	pw_conv1 = puf_layer(aw_conv1, pufs_conv1, ratio, "conv1");
	print "end1"
	pw_conv2 = puf_layer(aw_conv2, pufs_conv2, ratio, "conv2");
	print "end2"
	pw_conv3 = puf_layer(aw_conv3, pufs_conv3, ratio, "conv3");
	print "end3"
	pw_conv4 = puf_layer(aw_conv4, pufs_conv4, ratio, "conv4");
	print "end4"
	pw_conv5 = puf_layer(aw_conv5, pufs_conv5, ratio, "conv5");
	print"end5"
	print "puf end"

	#recoverunreliable
	print "recover"
	ow_conv1 = np.zeros(np.shape(w_conv1));
	ow_conv2 = np.zeros(np.shape(w_conv2));
	ow_conv3 = np.zeros(np.shape(w_conv3));
	ow_conv4 = np.zeros(np.shape(w_conv4));
	ow_conv5 = np.zeros(np.shape(w_conv5));

	ow_conv1 = pufunlayer(pw_conv1, pufs_conv1, ratio, "conv1");
	print "end1"
	ow_conv2 = pufunlayer(pw_conv2, pufs_conv2, ratio, "conv2");
	print "end2"
	ow_conv3 = pufunlayer(pw_conv3, pufs_conv3, ratio, "conv3");
	print "end3"
	ow_conv4 = pufunlayer(pw_conv4, pufs_conv4, ratio, "conv4");
	print "end4"
	ow_conv5 = pufunlayer(pw_conv5, pufs_conv5, ratio, "conv5");
	print "end5"

	net.params['conv1'][0].data[...] = ow_conv1;
	net.params['conv2'][0].data[...] = ow_conv2;
	net.params['conv3'][0].data[...] = ow_conv3;
	net.params['conv4'][0].data[...] = ow_conv4;
	net.params['conv5'][0].data[...] = ow_conv5;
	print"recover end"
